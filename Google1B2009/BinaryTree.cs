...
//The Binary tree struct + the Prob calculation function in C#
public class Node
{
    ...
    public Node(double weight, string name)
    {
        this.Weight = weight;
        this.Name = name;
    }
...
//Class used to make the graphical prints of the trees (taken from Stack Overflow but edited to match our specs)
public static class TreePrinter
{
    ...
    public static void Print(this Node root, int topMargin = 2, int leftMargin = 2)
    {
        ...
        {
            var item = new NodeInfo { Node = next, Text = next.Weight.ToString(" 0.00000 ")  };
            if (next.Name != null)
                item.Text +=  " " + next.Name;
            if (level < last.Count)
            {
